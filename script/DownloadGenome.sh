...
function WithoutCheckSums(){
	filelist="$( cat weblist | egrep "Gorilla_gorilla.gorGor(.*).dna.chromosome.[^MT]" )"
...
function WithCheckSums(){
	...
	`cat checksums | egrep "Gorilla_gorilla.gorGor(.*).dna.chromosome.[^MT]" > fastalist`
...
DIRNAME=${Path}Refgenome_${version}
`mkdir ${DIRNAME}`
# for saving the files and using local paths.
cd ${DIRNAME}
wget ${BaseUrl}
`cat index.html | egrep "a href" | awk -F "<" '{ print $2 }' |  awk -F '"' '{ print $2 }' > weblist `
if [ `cat weblist | egrep "CHECKSUMS" ` ]
then
